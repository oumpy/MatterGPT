...
def parse_args():
    ...
    parser.add_argument('--gpt-model', default=os.environ.get('MATTERGPT_GPT_MODEL', 'gpt-3.5-turbo', help='OpenAI ChatGPT model to use')
    ...
    parser.add_argument('--outgoing-webhook-token', default=os.environ.get('MATTERGPT_OUTGOING_WEBHOOK_TOKEN', help='Mattermost outgoing webhook token')
    parser.add_argument('--system-message', default=os.environ.get('MATTERGPT_SYSTEM_MESSAGE', 'You are ChatGPT, a large language model trained by OpenAI.', help='System message for ChatGPT to influence its behavior')
    parser.add_argument('--additional-message', default=os.environ.get('MATTERGPT_ADDITIONAL_MESSAGE', '', help='Message appended to user message before sending to ChatGPT')
    ...
    parser.add_argument('--max-tokens', type=int, default=int(os.environ.get('MATTERGPT_MAX_TOKENS', 1000, help='Maximum tokens for the generated text')
    parser.add_argument('--temperature', type=float, default=float(os.environ.get('MATTERGPT_TEMPERATURE', 0.5, help='Sampling temperature for ChatGPT responses')
    parser.add_argument('--top-p', type=float, default=float(os.environ.get('MATTERGPT_TOP_P', 1.0, help='Top-p (nucleus sampling) parameter')
    parser.add_argument('--frequency-penalty', type=float, default=float(os.environ.get('MATTERGPT_FREQUENCY_PENALTY', 0.0, help='Penalty for repeated tokens (float from -2.0 to 2.0)')
    parser.add_argument('--presence-penalty', type=float, default=float(os.environ.get('MATTERGPT_PRESENCE_PENALTY', 0.0, help='Penalty for introducing new topics (float from -2.0 to 2.0)')
    parser.add_argument('--max-thread-posts', type=int, default=int(os.environ.get('MATTERGPT_MAX_THREAD_POSTS', 0, help='Maximum number of posts from thread history (0 means unlimited)')
    parser.add_argument('--max-thread-tokens', type=int, default=int(os.environ.get('MATTERGPT_MAX_THREAD_TOKENS', 4096, help='Maximum number of tokens from thread history')
    parser.add_argument('--debug', action='store_true', default=os.environ.get('MATTERGPT_DEBUG', 'false').lower() == 'true', help='Enable debug mode for Flask')
    parser.add_argument('--flush-logs', action='store_true', default=os.environ.get('MATTERGPT_FLUSH_LOGS', 'false').lower() == 'true', help='Immediately flush log output')
    parser.add_argument('--gunicorn-path', default=os.environ.get('MATTERGPT_GUNICORN_PATH', help='Path to Gunicorn executable for production server')
    parser.add_argument('--workers', type=int, default=int(os.environ.get('MATTERGPT_WORKERS', 1, help='Number of Gunicorn worker processes')
    parser.add_argument('--timeout', type=int, default=int(os.environ.get('MATTERGPT_TIMEOUT', 30, help='Gunicorn timeout in seconds')
