...
def parse_args():
    ...
    parser.add_argument('--mm-url', default=os.environ.get('MATTERMOST_MM_URL') or 'localhost', help='Mattermost server URL')
    parser.add_argument('--mm-port', type=int, default=os.environ.get('MATTERMOST_MM_PORT') or 443, help='Mattermost server port')
    parser.add_argument('--mm-scheme', default=os.environ.get('MATTERMOST_MM_SCHEME') or 'https', help='Mattermost server scheme (http or https)')
    parser.add_argument('--webhook-port', type=int, default=os.environ.get('MATTERMOST_WEBHOOK_PORT') or 5000, help='Webhook listening port')
    parser.add_argument('--gpt-model', default=os.environ.get('MATTERMOST_GPT_MODEL') or 'gpt-3.5-turbo', help='OpenAI ChatGPT model')
    parser.add_argument('--logfile', default=os.environ.get('MATTERMOST_LOGFILE'), help='Path to log file (default: stdout)')
    parser.add_argument('--loglevel', default=os.environ.get('MATTERMOST_LOGLEVEL') or 'INFO', help='Logging level (DEBUG, INFO, WARNING, ERROR, CRITICAL)')
    parser.add_argument('--max-tokens', type=int, default=os.environ.get('MATTERMOST_MAX_TOKENS') or 100, help='Maximum tokens for the generated text')
    parser.add_argument('--temperature', type=float, default=os.environ.get('MATTERMOST_TEMPERATURE') or 0.5, help='Temperature for the generated text (higher values make the output more diverse, lower values make it more conservative)')
    parser.add_argument('--max-thread-posts', type=int, default=os.environ.get('MATTERMOST_MAX_THREAD_POSTS') or 20, help='Maximum number of posts to fetch in a thread')
